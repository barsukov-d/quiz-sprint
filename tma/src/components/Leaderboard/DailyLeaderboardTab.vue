...
import { useGetDailyChallengeLeaderboard } from '@/api/generated/hooks/dailyChallengeController/useGetDailyChallengeLeaderboard'
...
const { data, isLoading, isError, error, refetch } = useGetDailyChallengeLeaderboard(
	computed(() => ({ limit: 50, playerId: currentUser.value?.id ?? undefined })),
)

const entries = computed(() => data.value?.data.entries ?? [])
const totalPlayers = computed(() => data.value?.data.totalPlayers ?? null)
const playerRank = computed(() => data.value?.data.playerRank ?? null)
...
<template>
	<LeaderboardList
		...
		:empty-description="t('leaderboard.daily.emptyDesc')"
	...
	>
		...
		<template #entries>
			...
			>
				<template #stats>
					<p class="text-sm font-bold text-primary">{{ entry.score }}</p>
					<p class="text-xs text-(--ui-text-dimmed)">{{ t('leaderboard.pts') }}</p>
				</template>
			</LeaderboardEntryRow>
		</template>
	</LeaderboardList>
